...
class poem:
	def __init__(self,lines):
		self.lines = lines # list containing lines from poem
		self.vocabulary = {} # dictionary containing all words appearing in poem
		...
		# vocabulary[word] = [word_index,count], count is the number of appearances
		# word_index will be the corresponding index of the vector representation
		word_index = 0
		for line in lines:
	...
	def best_match(self, input_string):
		...
		for target in self.line_vectors:
			...
			for i in non_zero_indices:
				if target[i] != 0:
					# similarity_score += 1 # this option disregards repeated words
					similarity_score += min(target[i],input_vector[i])
